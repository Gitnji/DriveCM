...
<div class="mx-auto max-w-2xl">
    ...
    <h1 class="mt-3 text-2xl font-bold text-neutral">Edit staff member</h1>
    <p class="mt-1 text-sm text-neutral/60">Email cannot be changed. To use a different email, remove this account and create a new one.</p>
    ...
    <form method="POST" action="{{ route('lms.staff.update', $staff) }}" class="mt-6 space-y-5 rounded-xl border border-neutral/10 bg-white p-6">
        @csrf @method('PUT')
        ...
        <div>
            ...
            <select id="role" name="role" required
                    class="mt-1 w-full rounded-lg border border-neutral/20 px-3 py-2 text-sm focus:border-primary focus:ring-1 focus:ring-primary">
                <option value="instructor" @selected(old('role', $staff->role) === 'instructor')>Instructor</option>
                <option value="secretary" @selected(old('role', $staff->role) === 'secretary')>Secretary</option>
            </select>
        </div>
        ...
        <div>
            ...
            <select id="language" name="language" required
                    class="mt-1 w-full rounded-lg border border-neutral/20 px-3 py-2 text-sm focus:border-primary focus:ring-1 focus:ring-primary">
                <option value="en" @selected(old('language', $staff->language) === 'en')>English</option>
                <option value="fr" @selected(old('language', $staff->language) === 'fr')>Français</option>
            </select>
        </div>
